...
#统计关系表d_event里面元素和他的频率
    ...
    for i in feature_list:
        ...
        vector = np.hstack((vector, tempvec))#=========这个地方特征向量可以加入nlp里面embedding,可以 不止用jaccard.*******************************************************************************或者单纯one-hot编码.等方式都可以.
...
def cross_val(feature,label,event_num):
    ...
    for train_index, test_index in skf.split(feature, label):#进行数据4:1分割.
